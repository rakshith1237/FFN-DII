...
import { requirePersona, getPersonaCode, getTenantId, getUser } from '@/lib/auth/session'
...
export default async function PartnerLayout({ children }: { children: ReactNode }) {
  ...
  const [personaCode, tenantId, user] = await Promise.all([getPersonaCode(), getTenantId(), getUser()])
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB]">
      <TopNav orgName={tenant?.name ?? undefined} userId={user?.id} />
